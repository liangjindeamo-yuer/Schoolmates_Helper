...
def task_square(request):
    ...
    tasks_list = Task.objects.filter(is_pickedup=False).exclude(publisher_id=username)
    data = {
...
def task_square_sort(request, type_id, order):
    ...
    if type_id != 0:
        ...
        sort='全部任务'
        tasks_list = Task.objects.filter(is_pickedup=False).order_by(order).exclude(publisher_id=username)
